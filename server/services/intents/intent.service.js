...
const processIntent = (apiResponse, session) => {
  ...
  // ==================================================  your code here
  // Message from user
  // console.log('=====================', apiResponse.result.resolvedQuery);

  // Bot response
  // apiResponse.result.fulfillment.speech = 'No, Im a robot';
  // if (apiResponse.result.action === 'maps.search') {
...
const getResponse = (message, sessionId, identifier, channel) => {
  ...
  return (
    analyticsService
      .getLastSessionForUser(identifier, sessionId)
      ...
      .then(result => {
        ...
        return analyticsService.saveResponse(
          result.question,
          result.answer,
          channel,
          identifier,
